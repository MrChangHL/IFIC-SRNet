...
def deconv_sisr_L2_fft(y, k, scale, gamma=0.01, eps=1e-2):
    ...
    temp = torch.fft.fft2(STy, dim=(-2, -1), norm=None)
    temp = torch.stack((temp.real, temp.imag), -1)  
    FBFy = cmul(FBC, temp)
    ...
    Xest = torch.fft.irfft2(torch.complex(FX[..., 0], FX[..., 1]), dim=(-2,-1))
    # Xest = torch.fft.ifft2(FX, dim=(-2, -1))
