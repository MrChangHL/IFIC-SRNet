...
class KernelNet(nn.Module):
    def __init__(self, ksize=13):
        ...
        self.global_pool = nn.Sequential(
            nn.AdaptiveAvgPool2d(self.ksize)   # (N, C, H, W)  -->  (N, C, ksize, ksize)
        )
    ...
    def forward(self, input_tensor):
        b, c, h, w = input_tensor.size()  # b 3 64 64
        out = self.in_conv(input_tensor)  # b 64 64 64
        out = self.body1(out)  # b 64 64 64

        out = self.global_pool(out)  # b 64 13 13

        out = self.body2(out)   # b 1 13 13
