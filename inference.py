...
class Inference:
    def __init__(self, args):
        ...
        if self.large_model:
            self.net = PWC_Recons(
                ...
                # n_colors=3, n_sequence=5, extra_RBS=5, recons_RBS=20, n_feat=128, scale=4, device=self.device     #
            )
        else:
            self.net = PWC_Recons(
    ...
    def infer(self):
        with torch.no_grad():
            ...
            # input = torch.randn(1, 5, 3, 128, 128).cuda()
            # inputss = {'x': input}
            # model_small = self.net = PWC_Recons(
            #     n_colors=3, n_sequence=5, extra_RBS=1, recons_RBS=10, n_feat=128, scale=4, device=self.device
            # )
            # model_small = model_small.cuda()
            # # model_10 = self.net = PWC_Recons(
            # #     n_colors=3, n_sequence=5, extra_RBS=1, recons_RBS=10, n_feat=128, scale=4, device=self.device
            # # )
            # # print_network(model_small)
            # flops, param = profile(model_small, inputs=(inputss, ))
            # print('flops and param', flops/1e9, param/1e6)
            ...
            for v in videos:
                ...
                for in_seq, gt_seq in zip(input_seqs, gt_seqs):                      # 保存测试的结果
                    # print(0)
                    ...
                    sum_time += (forward_time - preprocess_time)
            # print(sum_time/100)
            print(sum_time/(100 * 12))
            self.logger.write_log('per_frame_time:{:.3}s'.format(sum_time/(100 * 12)))
            ...
            # self.logger.write_log("# Total AVG-PSNR={:.5}, AVG-SSIM={:.4}".format(sum_psnr / ((k+1) * len(total_psnr[k])), sum_ssim / ((k+1) * len(total_psnr[k]))))
    ...
    def get_PSNR_SSIM(self, output, gt, crop_border=8):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_path', type=str,
                        # default='/home/chl/桌面/exp_result/test_dataset/val20/sharp',
                        default='/home/chl/PycharmProjects/BSVSR/dataset/val_20_BlurDown_Gaussian/blurdown_x4',
                        help='the path of input')
    parser.add_argument('--gt_path', type=str,
                        # default='/home/chl/桌面/exp_result/test_dataset/val20/sharp',
                        default='/home/chl/PycharmProjects/BSVSR/dataset/val_20_BlurDown_Gaussian/sharp',
                        help='the path of input')
    parser.add_argument('--model_path', type=str,
                        default='../pretrain_models/sig20/SFTLayer/0_no_conv_repeat_Adam/model_80.pt',
                        help='the path of pretrain model')


    parser.add_argument('--result_path', type=str, default='../result/DBVSR/sig20/SFTLayer/HAT_HAB/0_no_conv_repeat_Adam/', help='the path of result')
    ...
    parser.add_argument('--large_model', default=True, action='store_true', help='using large model if true')
